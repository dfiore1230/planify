...
fun EventsListScreen(navController: NavController) {
    ...
    LaunchedEffect(instance?.id) {
        ...
        isLoading = true
        try {
            events = repositories.eventRepository.listEvents(instance)
            errorMessage = null
        } catch (e: Exception) {
    ...
    Scaffold(
        floatingActionButton = {
            if (instance != null) {
                FloatingActionButton(onClick = { navController.navigate("events/new") }) {
        ...
        Column(modifier = Modifier.fillMaxSize().padding(padding)) {
            ...
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                items(events) { event ->
                    EventRow(event = event, timeZoneId = appSettings.timeZoneId) {
                        navController.navigate("events/detail/${event.id}")
                    }
                }
            }
        }
...
@Composable
private fun EventRow(event: Event, timeZoneId: String, onClick: () -> Unit) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .clickable { onClick() }
        ...
        Text(DateFormatters.formatDisplay(event.startAt, timeZoneId), style = MaterialTheme.typography.bodySmall)
        Text(event.venueDisplayDescription, style = MaterialTheme.typography.bodySmall)
    }
}
