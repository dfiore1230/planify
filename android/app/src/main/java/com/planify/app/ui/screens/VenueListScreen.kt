...
fun VenueListScreen(navController: NavController) {
    ...
    var items by remember { mutableStateOf<List<VenueDetail>>(emptyList()) }
    ...
    LaunchedEffect(instance?.id) {
        ...
        isLoading = true
        try {
            items = repositories.venueRepository.fetchAll(instance)
            errorMessage = null
        } catch (e: Exception) {
    ...
    Scaffold(
        floatingActionButton = {
            if (instance != null) {
                FloatingActionButton(onClick = { navController.navigate("venues/new") }) {
            ...
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                items(items) { venue ->
                    Column(
                        modifier = Modifier
                            .fillMaxWidth()
                            .clickable { navController.navigate("venues/detail/${venue.id}") }
                            .padding(16.dp)
                    ) {
                        Text(venue.name, style = MaterialTheme.typography.titleMedium)
                        Text(venue.displayAddress, style = MaterialTheme.typography.bodySmall)
                    }
                }
            }
        }
